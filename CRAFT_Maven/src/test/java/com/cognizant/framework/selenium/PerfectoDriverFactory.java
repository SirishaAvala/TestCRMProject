...
public class PerfectoDriverFactory {
	...
	public static AppiumDriver getPerfectoAppiumDriver(MobileExecutionPlatform executionPlatform, String deviceName,
			String perfectoURL, SeleniumTestParameters testParameters) {
		...
		try {
			...
			case WEB_ANDROID:
				...
				desiredCapabilities.setCapability("platformName", "Android");
				if (testParameters.getIsDeviceUdid()) {
					desiredCapabilities.setCapability("deviceName", testParameters.getPerfectoDeviceId());
				} else {
					desiredCapabilities.setCapability("manufacturer", testParameters.getManuFacturerName());
					desiredCapabilities.setCapability("model", testParameters.getModelName());
				}
	...
	public static WebDriver getPerfectoRemoteWebDriver(MobileExecutionPlatform executionPlatform, String deviceName,
		...
		try {
			...
			case ANDROID:
				...
				desiredCapabilities.setCapability("appActivity",
						mobileProperties.getProperty("Application_MainActivity_Name"));
				try {
					driver = new RemoteWebDriver(new URL(perfectoURL), desiredCapabilities);
				} catch (MalformedURLException e) {
					throw new FrameworkException(
